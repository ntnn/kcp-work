...
again later when the deps are updated.
    ...
    git add .
    git commit -m 'drop: add kcp dependencies'
...
And update the go.mod files and vendor directories:
    ...
    git add .
    git commit -m 'drop: update vendor'
...
redirect to the local forks

    git add .
    git commit -m 'drop: redirect to local forks'

After everything is cherry-picked and the PRs in the other repos are
merged update the depenencies, update the vendor directory and commit.
